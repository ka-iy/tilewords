...
// ChooseMove selects and returns the CPU's move for the current turn.
// It always returns a non-nil engine.Move (NFR-AI-R5).
//
// Algorithm (BR-AI-10):
//  1. Generate all legal moves for the CPU rack.
...
//  4. Otherwise pass.
//
// ChooseMove always operates on state.CPURack — the human rack is never consulted (BR-AI-08).
func ChooseMove(
	...
	candidates := GenerateMoves(state.Board, state.CPURack, dict)
	...
	if state.CPURack.Count() > 0 && state.Bag.Count() >= engine.MaxRackSize {
		return engine.ExchangeMove{Tiles: state.CPURack.Tiles()}
	}
