...
// most-recent-move points, prefixed with "You"/"CPU" — the human's in green (Success), the
// CPU's in amber (Warning), separated by " / " — that a play clears a transient message,
...
func TestStatusSummary_ShowsBothPlayerPoints(t *testing.T) {
	...
	if txt, col := segText(t, segs[2]); txt != "CPU —" || col != string(theme.ColorNameWarning) {
		t.Errorf("initial CPU segment = %q/%s, want \"CPU —\"/warning", txt, col)
	}
	...
	gs.logCommand("CPU", &engine.PlayCommand{Move: engine.PlayMove{WordsFormed: []string{"QI"}, Score: 22}})
	...
	if txt, col := segText(t, segs[2]); txt != "CPU +22" || col != string(theme.ColorNameWarning) {
		t.Errorf("CPU segment = %q/%s, want \"CPU +22\"/warning", txt, col)
	}

	// A pass counts as +0 for the human; the CPU's last play stands.
	...
	if txt, _ := segText(t, segs[2]); txt != "CPU +22" {
		t.Errorf("after human pass, CPU segment = %q, want \"CPU +22\" (unchanged)", txt)
	}

	// An exchange also counts as +0.
	gs.logCommand("CPU", &engine.ExchangeCommand{Move: engine.ExchangeMove{Tiles: []engine.Tile{{Letter: 'A'}, {Letter: 'B'}}}})
	if txt, _ := segText(t, gs.statusSegments()[2]); txt != "CPU +0" {
		t.Errorf("after CPU exchange, CPU segment = %q, want \"CPU +0\"", txt)
	}
}

// TestStatusSummary_ThinkingAndErrorOverride verifies CPU-thinking and error messages take
...
func TestRecomputeLastPoints_FromHistory(t *testing.T) {
	...
	if gs.lastHumanPts != -1 || gs.lastCPUPts != -1 {
		t.Fatalf("empty history: got human=%d cpu=%d, want -1/-1", gs.lastHumanPts, gs.lastCPUPts)
	}

	// points mirrors what logCommand stores: a play's score, 0 for a pass.
	gs.history = []historyEntry{
		{player: "You", points: 30},
		{player: "CPU", points: 15},
		{player: "You", points: 0},
	}
	gs.recomputeLastPoints()
	if gs.lastHumanPts != 0 || gs.lastCPUPts != 15 {
		t.Errorf("after pass: got human=%d cpu=%d, want human=0 (pass) cpu=15", gs.lastHumanPts, gs.lastCPUPts)
	}
